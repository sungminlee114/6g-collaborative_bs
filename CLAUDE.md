...
**모든 의미 있는 작업은 반드시 `assets/backlog.json`에 기록해야 한다.**
이것은 선택이 아니라 필수이다. 작업 시작 전에 먼저 기록하라.

```
작업 시작 → /task start <type>/<name> "설명"
작업 완료 → /task done "요약"
이미 끝난 작업 → /task log <type>/<name> "요약"
작업 실패 → /task fail "사유"
```

**기록 대상:** 코드 구현/수정, 조사/분석, 설정 변경, 실험 실행, 버그 수정
**기록 제외:** 단순 질문 응답, 파일 읽기만, 한두 줄 수정
